...
representation? The structural unit here isn't classical algebra; it's the
**doubling-pair cluster** observed in trained runs.
...
quantities this playbook measures on top of it:

1. **Helper multipliers.** Which ×m relations appear between helpers and
   primaries (ReLU's harmonic structure, `a₂ ≠ 0 / a₃ = 0`, predicts which
   are available).
2. **Output-neuron cost of helpers.** Dedicated cluster size of helper
   characters versus their primaries.
3. **Cost per primary.** How neuron allocation per primary scales with
   `d_mlp` and with character order.
...
# Aggregate → cost plot: x=d_mlp, y=neurons; color/marker by order(k).
# Look for systematic dependence on d_mlp and on order(k).
```
