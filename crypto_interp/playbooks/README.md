...
- **Mech-interp structure (what lives on top).** Capacity-constrained
  representational economy is not classical algebra — it only exists in the
  trained network. The doubling-pair cluster `{χ_k, χ_{2k}}`, per-primary
  neuron budgets, free-rider helpers via ReLU's `a₂ ≠ 0` — these are the
  units the harness measures. Playbooks make them first-class verbs
