...
Aggregate two ways:
    1. Per-example margin distribution — where does the bulk of (a, b)
       pairs sit, and how heavy is the negative tail?
    2. Per-token error attribution — error rate per input token value,
       split by whether the token appeared in the A or the B position
       (an A↔B-symmetric pattern implicates the embedding column rather
       than position-dependent circuitry).

Also reports the kernel margin κ(0) − max_{d≠0} κ(d) per seed: the
symmetric kernel's own headroom, constant across (a, b).
